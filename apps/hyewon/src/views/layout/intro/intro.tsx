...
const Intro = () => {
	return (
		<div className="w-[460px] h-screen fixed py-26 px-30 flex flex-col gap-6 bg-[url('/side-panel-bg.png')]">
			...
			<Typography variants='display-l'>{aboutMyProfile.name}</Typography>
			<Typography variants='heading-m'>{aboutMyProfile.position}</Typography>
			<ul>
				<li>
					{aboutMyProfile.summaryTxt.map((i, idx) => (
						<Typography variants='body-m' key={idx} className='flex gap-1 items-center py-2'>
